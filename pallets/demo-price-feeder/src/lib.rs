...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: BlockNumberFor<T>) {
			...
			// // 尝试从 offchain storage 读取 keystore
			// let _keystore_content = match offchain::local_storage_get(StorageKind::PERSISTENT, b"cyferio-keystore") {
			// 	Some(content) => content,
			// 	None => {
			// 		log::error!("Cyferio keystore not found in offchain storage");
			// 	}
			// };

			// // 使用 keystore_content 进行后续操作
			// log::info!("Cyferio keystore loaded successfully");
			...
			log::info!("===== test_sui start =====");
			let gas_price = Self::test_sui();
			log::info!("gas_price: {:?}", gas_price);
			log::info!("===== test_sui end =====");
...
impl<T: Config> Pallet<T> {
	...
	fn choose_transaction_type(block_number: BlockNumberFor<T>) -> TransactionType {
		...
		let val = StorageValueRef::persistent(b"ocw::last_send");
	...
	fn test_sui() -> Result<String, http::Error> {
		...
		log::info!("8");
		let reference_gas_price = response_data.epoch.referenceGasPrice;
